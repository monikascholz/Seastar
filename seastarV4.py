...
def applyWaterShed(image, show=False):
    ...
    labels = watershed(-distance, markers, mask=image, compactness=10)
    if show:
...
def findObjects(trackIm0, paramDict, flag, show=False):
    ...
    trackImTmp = np.abs(trackIm0)
    #trackImTmp = trackIm0
    width, height = trackIm0.shape
    trackIm = gaussian(trackImTmp, sigma = 3)
    # block out strange area in SS2
    if flag =='SS2':
        #trackImTmp= trackIm
        trackImTmp[70:145,530:610] = 0
        
    #trackIm = median(trackImTmp, disk(2))
    #trackIm = trackImTmp
    #thresh = np.percentile(trackIm,[100-100/width*height])
    # apply threshold
    thresh = threshold_yen(trackImTmp)
    
    #bw = closing(trackIm > thresh, square(5))
    ...
    # 
    #label_image = applyWaterShed(cleared, show)
    # define regions
...
def calculateBackground(impath, condition, analysisPath, paramDict, flag, show_figs = True):
    ...
    print time, paramDict['start'],len(smallSet)
...
def linkingTrajectories2D(impath, impath2, analysisPath, tracks, paramDict, condition):
    ...
    for imIndex, imFile in enumerate(allImFiles):
        ...
        if imIndex ==0 or np.any(nLost>paramDict['lostFrames']):
            ...
            currPos = initializePos(firstIm, paramDict, linked[imIndex-paramDict['lostFrames']*2:imIndex], ss2Im)
            nLost = np.ones(paramDict['NStars'])
        if imIndex%10==0:
            print 'Linking frame ', imIndex
        ...
        for loc, value in matches:
            nStar, coordStar = loc
            if value < paramDict['minDistance']*int(np.min([10,nLost[nStar]])):
                # append to coordinate
                ...
                if len(coords) > 1:
                    # don't interrupt if we can't see anything
                    nLost[nStar]+=1
...
#    imPath = '/media/monika/MyPassport/Qs/{}{}'
    #imPath = '/media/monika/MyPassport/Qs/{}{}Qs'
...
#    
    ...
    if linking:
        ...
        linkingTrajectories2D(imPath.format('SS1', condition),imPath.format('SS2', condition), analysisPath, tracks,  paramDict, condition)
        # write status
        writeStatus(os.path.join(analysisPath, '{}_status.txt'.format(condition)), \
